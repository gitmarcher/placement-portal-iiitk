...
const CoordinatorDrive = () => {
  ...
  const getDriveActionButton = (drive) => {
    if (!drive.isActive) {
      ...
      // Check if results have started to determine which action to allow
      if (drive.results_started) {
        // Results have started, can only end the drive
        return {
          text: "End Drive",
          action: "end-drive",
        ...
        // Results haven't started, allow resuming applications
        return {
          text: "Resume Applications",
          action: "resume-applications",
                ...
                {selectedDrive ? (
                  <>
                    ...
                    <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 mt-4 mb-8 px-2 sm:px-4">
                      ...
                      {/* Show End Drive button when applications are closed but results haven't started */}
                      {selectedDrive.isActive &&
                        !selectedDrive.acceptingApplications &&
                        !selectedDrive.results_started && (
                          <button
                            className={`w-full sm:w-auto px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-medium shadow-sm transition-all duration-200 ${
                              toggleLoading
                                ? "opacity-50 cursor-not-allowed"
                                : ""
                            } order-3`}
                            onClick={() => handleDriveAction("end-drive")}
                            disabled={toggleLoading}
                          >
                            {toggleLoading ? (
                              <span className="flex items-center justify-center">
                      ...
                      <div
                        className="w-full sm:w-auto flex rounded-md px-3 py-2 items-center justify-center cursor-pointer hover:bg-gray-100 transition-all duration-200 border border-gray-200 order-4"
